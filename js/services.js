/* services.js
   Geocoding and routing (Nominatim, ViaCEP, OSRM) encapsulated as async functions.
*/
...
export async function calcularDistancia(origem, destino) {
    ...
    if (isCep(origem)) {
        ...
        if (addrObj) {
            ...
            resolvedOrig = { cep: addrObj.cep, bairro: addrObj.bairro, cidade: addrObj.localidade, uf: addrObj.uf, full: addrObj.formatted };
        } else return null;
    }
    if (isCep(destino)) {
        ...
        if (addrObj) {
            ...
            resolvedDest = { cep: addrObj.cep, bairro: addrObj.bairro, cidade: addrObj.localidade, uf: addrObj.uf, full: addrObj.formatted };
        } else return null;
    }
    ...
    if (distanciaOSRM) return { km: distanciaOSRM, origResolved: resolvedOrig, destResolved: resolvedDest };
